...
const Button1 = () => {
  ...
  let button1Time = 1000;
  let button2Time = 5000;
  ...
  const button1Data = useCallback(async (url, delay) => {
      setLoading(true);
    ...
    <>
      <div className="flex justify-between bg-blue-600 p-4 mx-2">
        ...
        <button onClick={setToggle} className="flex px-4 py-2 rounded overflow-hidden">
          <span className={`flex-1 px-2 py-1 text-center ${   on ? "bg-white text-black" : "bg-black text-white" }`}
          >{on ? "ON" : "OFF"}</span>
          <span className={`flex-1 px-2 py-1 text-center ${   on ? "bg-black text-white" : "bg-white text-black" }`}>
            {on ? "OFF" : "ON"}</span>
        </button>
      </div>
      <div className="flex mt-4 space-x-4 mx-2 justify-between">
        <Buttons button="A" onClick={() => button1Data(url0, button1Time)}/>
        <Buttons button="B" onClick={() => button1Data(url1, button2Time)}/>
